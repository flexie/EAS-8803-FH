...
# Part I — Foundations {background-color="#2c3e50"}
...
| Born approximation | Reflectivity | Scattered wavefield |
...
**Orthogonal matrices** satisfy $\mathbf{A}^\top\mathbf{A} = \mathbf{A}\,\mathbf{A}^\top = \mathbf{I}$, so the adjoint *is* the inverse.
...
# Part II — Least-Squares Inversion {background-color="#2c3e50"}
...
# Part III — SVD & Generalized Inverses {background-color="#2c3e50"}
...
## The Moore–Penrose Pseudo-Inverse{.smaller}
...
# Part IV — Regularization {background-color="#2c3e50"}
...
## Regularization as a Bias–Variance Trade-off{.smaller}
...
# Part V — Application {background-color="#2c3e50"}

## Deconvolution — The Setup{.smaller}
...
## Deconvolution — Why It's Ill-Posed{.smaller}
