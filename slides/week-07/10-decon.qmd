...
## Seismic convolutional model{.smaller}
...
## Convolutional model — geological picture{.smaller}
...
## Example: reflectivity $\ast$ Ricker wavelet{.smaller}
...
## Ricker wavelet and its amplitude spectrum{.smaller}
...
## Vibroseis: sweep & Klauder wavelet{.smaller}
...
## Objective of deconvolution{.smaller}
...
## Wiener filter for different $\varepsilon${.smaller}
...
<!-- $$\min_{\mathbf{f}} \|\mathbf{r} - \mathbf{f}\ast\mathbf{d}\|_2^2 $$
...
## Predictive deconvolution — motivation{.smaller}
...
- the source signal itself has a repetitive character,  e.g. the airgun bubble effect
-  certain layer generates strong reverberations,   e.g. a shallow water layer
...
![adapted from Eric Verschuur](figures/airgun_bubble.jpeg)

## Ocean bottom multiples

![adapted from Eric Verschuur](figures/waterbottom.png)

## Ocean bottom multiple removal

![adapted from Eric Verschuur](figures/waterbottom2.png)
...
## Predictive decon — least-squares objective{.smaller}
...
## Example: predictive deconvolution — shot record{.smaller}

![Adapted from Eric Verschuur](figures/predictive_decon_shotrecord.png){fig-align="center" width="85%"}
...
## Predictive decon — autocorrelation check{.smaller}
...
## $\ell_1$ deconvolution — practical aspects{.smaller}

The $\ell_1$ problem is **convex** but not differentiable at zero. It can be solved by:

- **ISTA** (Iterative Soft Thresholding [@daubechies2016sparsity]): $\mathbf{r}^{k+1} = \mathcal{S}_{\lambda}\!\left(\mathbf{r}^k + \mathbf{W}^T(\mathbf{d} - \mathbf{W}\mathbf{r}^k)\right)$
